...
Generated: 2026-03-01T00:14:56.605294+00:00
...
## 1. Layer A: Tool Interface Contract（v1.0.3）
...
- `describe capabilities --format json` は次を満たすこと（MUST）
  - `docs/specs/v1.0.3/doc-cli-contract_v1.0.3_capabilities-schema.md`
- `describe capabilities --format md` は次のテンプレートに従うこと（MUST）
  - `docs/specs/v1.0.3/doc-cli-contract_v1.0.3_capabilities-md-template.md`
...
- `describe params --format md|json` は次を満たすこと（MUST）
  - `docs/specs/v1.0.3/doc-cli-contract_v1.0.3_describe-params.md`
- `describe ai-authoring --format md|json` は次を満たすこと（MUST）
  - `docs/specs/v1.0.3/doc-cli-contract_v1.0.3_describe-ai-authoring.md`
...
- `docs/specs/v1.0.3/doc-cli-contract_v1.0.3_validate-schema.md`
...
- このリポジトリ内の `docs/specs/v1.0.3/` を参照すること。
...
## 1.4 SHOULD Topics (v1.0.3)

The following describe topics are RECOMMENDED for tools operating
in multi-tool environments. These topics are OPTIONAL but strongly encouraged.

### describe ls (SHOULD)

Purpose:
- Enumerate built-in guidance assets and vocabularies.
- Provide logical identifiers (sections, entities, occurrences, vocab).
- MUST NOT expose physical file paths.

### describe routing (SHOULD)

Purpose:
- Expose the scope of problems the tool is intended to solve.
- Clarify non-goals and complementary tools.
- Enable machine-discoverable tool routing.

If implemented, the topic MUST be declared in `describe capabilities`.

---

## 1.5 Routing & Language Discovery (v1.0.3)

Tools operating in multi-tool environments SHOULD expose:

- Routing discovery entry points
- Supported language inventory

Routing is defined as:

> The ability for a caller (human or automation) to discover
> whether a tool applies to a given problem context.

Routing details MAY be tool-specific, but the entry mechanism MUST
be discoverable via `describe capabilities`.

Language inventory SHOULD be declared when tools support
multi-language guidance or search behavior.
