...
## Routing & Domain Tagging Guidance (v1.0.3 Addition)

This version introduces non-breaking guidance for multi-tool routing.

### 1. anchorage.domain as Routing Tag (SHOULD)

- `anchorage.domain` MAY be used as a routing-oriented tag set.
- Domain values SHOULD be short, stable, lowercase identifiers.
- Domain values MAY correspond to tool capabilities such as
  `breakdown`, `merge`, `validate`, `iteration`, etc.
- Domain vocabularies are not globally fixed at the document layer.
  Tool-level contracts (Doc CLI Contract) MAY publish recommended sets.

### 2. Multi-language Guidance (SHOULD)

- A document MAY provide guidance in multiple languages.
- If multiple languages are mixed, `entities[].content_lang`
  SHOULD be explicitly set.
- Tool-level language discovery SHOULD be exposed via
  `describe capabilities`.

These additions do not change the data model and are fully backward compatible.

---

End of v1.0.3 additions.
