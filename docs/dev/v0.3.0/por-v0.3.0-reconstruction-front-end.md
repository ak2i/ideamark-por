# POR v0.3.0 Design Hypothesis — Precursor-based Reconstruction Front-end
...
Previous draft: `docs/dev/v0.2.1/por-v0.2.1-planning.md` (promoted and removed)

## 1. Purpose shift

v0.2.0 treated POR M1 as a text-to-IdeaMark generator. The first local Qwen3 4B
run showed that local extraction can produce many useful matches, but also
exposed a conceptual issue: the emitted IdeaMark should not be understood as a
completed meaning object.

v0.3.0 defines POR as a **Knowledge Reconstruction Front-end**.
...
A fragment must carry source anchors. It may carry weak or uncertain labels. It
should be small enough to retrieve and reprocess, but large enough that a later
system can reproduce why it was selected.
...
Skeleton Affordance is not directly dictionary-defined in v0.3.0.

SPs and MSs do not say which higher-level intellectual activity must occur. They
mark locations where such activity may be afforded. Projection and later
...
v0.3.0 introduces detector plugins. Each plugin should implement one or more of
these roles:
...
v0.3.0 does not replace existing Skeleton Families. Instead, Skeleton Families
...
Harmony-related controls are defined in
...
The current local baseline is Ollama with Qwen3 4B. For v0.3.0, smaller models
...
- v0.3.0 should prioritize fast SP/MS extraction, fragment anchoring, and
...
6. How should multiple detector outputs be merged when rule, tiny model, and LLM
   detectors disagree?
7. What is the minimum local benchmark suite for v0.3.0?
...
- add Projection controls for resolution, recall bias, boundary policy, detector
  policy, epistemic policy, and Harmony policy;
- compare projections over the same source using fragment distributions and
